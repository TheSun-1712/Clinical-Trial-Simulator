...
```bash
python training/train_grpo.py --backend trl --config training/configs/grpo_smoke.yaml --output artifacts/policy/latest_llm_smoke.json
```

8GB NVIDIA GPU training:

```bash
python training/train_grpo.py --backend trl --config training/configs/grpo_gpu_8gb.yaml --output artifacts/policy/latest_llm.json
```

Hugging Face GPU-oriented one-shot workflow:

```bash
python scripts/run_hf_training.py --config training/configs/grpo_gpu_8gb.yaml --output artifacts/policy/latest_llm.json --episodes 12
```

Optional manifest upload to Hub (adapter upload is handled by `training/train_grpo.py` via `upload_repo` + `HF_TOKEN`):

```bash
export HF_TOKEN=hf_xxx
python scripts/run_hf_training.py --config training/configs/grpo_gpu_8gb.yaml --output artifacts/policy/latest_llm.json --upload-manifest-repo your-username/clinical-trial-manifests
```

Reward modes:

- `reward_mode: text` uses fast text-heuristic scoring of JSON validity/action quality.
- `reward_mode: env_rollout` reconstructs sampled simulator state metadata (seed + warmup action history) and scores the model action via real environment rollout plus validation penalties.
- `env_rollout` generally improves alignment with simulator dynamics, but it is slower because each completion triggers additional environment steps.
...
- For reproducible training metrics, see `artifacts/training/latest_metrics.json`.
